...
class TwoViewPipelineTWIN(BaseModel):
    default_conf = {
        "extractor0": {
            "name": None,
            "trainable": True,
    ...
    def _init(self, conf):
        if conf.extractor0.name:
            self.extractor0 = get_model(conf.extractor0.name)(to_ctr(conf.extractor0))
        if conf.extractor1.name:
            self.extractor1 = get_model(conf.extractor1.name)(to_ctr(conf.extractor1))
    ...
    def extract_view(self, data, i):
        if i == "0":
            ...
            if self.conf.extractor0.name and not skip_extract:
                pred_i = {**pred_i, **self.extractor0(data_i)}
            elif self.conf.extractor0.name and not self.conf.allow_no_extract:
                pred_i = {**pred_i, **self.extractor0({**data_i, **pred_i})}
        else:
            data_i = data[f"view{i}"]
            pred_i = data_i.get("cache", {})
            skip_extract = len(pred_i) > 0 and self.conf.allow_no_extract
            if self.conf.extractor1.name and not skip_extract:
                pred_i = {**pred_i, **self.extractor1(data_i)}
            elif self.conf.extractor1.name and not self.conf.allow_no_extract:
                pred_i = {**pred_i, **self.extractor1({**data_i, **pred_i})}            
        return pred_i
